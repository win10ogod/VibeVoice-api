...
class VibeVoiceForConditionalGenerationInference(VibeVoicePreTrainedModel, GenerationMixin):
    ...
    def noise_scheduler(self):
        # Use thread-local scheduler if set to ensure concurrency-safety
        get_tls = getattr(self.model, 'get_thread_local_noise_scheduler', None)
        if callable(get_tls):
            return get_tls()
       ...
       # Define all valid tokens that can be generated
        ...
        for step in progress_bar:
            ...
            if diffusion_start_indices.numel() > 0 and kwargs.get('refresh_negative', True):
                ...
                # Handle cache API changes: prefer layers[idx].keys/values; fallback to key_cache/value_cache
                pkv = negative_model_kwargs['past_key_values']
                try:
                    layers = pkv.layers
                    kv_pairs = [(layers[i].keys, layers[i].values) for i in range(len(layers))]
                except Exception:
                    kv_pairs = list(zip(getattr(pkv, 'key_cache', []), getattr(pkv, 'value_cache', [])))
                for layer_idx, (k_cache, v_cache) in enumerate(kv_pairs):
            ...
            if diffusion_indices.numel() > 0:
                ...
                if non_diffusion_mask.any():
                    ...
                    pkv2 = negative_model_kwargs['past_key_values']
                    try:
                        layers2 = pkv2.layers
                        kv_pairs2 = [(layers2[i].keys, layers2[i].values) for i in range(len(layers2))]
                    except Exception:
                        kv_pairs2 = list(zip(getattr(pkv2, 'key_cache', []), getattr(pkv2, 'value_cache', [])))
                    for layer_idx, (k_cache, v_cache) in enumerate(kv_pairs2):
                        # Process each non-diffusion sample
